...
const filteredTreks = trekData.filter(trek => {
  // Safely handle undefined/null, strings, and arrays
  const months = !trek.month 
    ? [] 
    : Array.isArray(trek.month) 
      ? trek.month 
      : [trek.month];
  
  // Check if months exist and find match
  return months?.some?.(
    m => m?.toLowerCase?.() === "february"
  ) ?? false;
});
...
<>
        ...
        <div className='w-full  max-w-4xl md:max-w-6xl  xl:max-w-7xl px-4 sm:px-6 md:px-8   mx-auto py-0 sm:py-2 lg:py-4 xl:py-10   bg-white text-center'>   
            ...
            <h1 className='text-xl sm:text-2xl  xl:text-4xl font-bold mb-0 md:mb-1 lg:mb-2 xl:mb-3 text-[#172541]'>Top February Treks for the Himalayas</h1>
        ...
        <div className='max-w-4xl md:max-w-6xl  xl:max-w-7xl  mx-auto px-4 md:px-8'>
          ...
          <div className="mb-4 md:mb-6 lg:mb-10  xl:mb-12">
            <h2 className="text-xl sm:text-2xl  xl:text-4xl font-bold mb-0  md:mb-1 lg:mb-2 xl:mb-3 text-[#172541]">Why Trek In February ?</h2>
